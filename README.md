...
Attach `useborage.com` and `www.useborage.com` to the project. Keep `www.useborage.com` assigned to Production and redirect the apex domain to `www` in Vercel’s domain settings. The repository marks the default Vercel subdomain `noindex` and sends `/sign-in` to the AWS-hosted application at `https://app.useborage.com/sign-in`.
